...
async function drawEgo(gid) {
  ...
  flowLayout(gid); applyRoleFilter();
...
async function drawChain(gid, perCol = 25) {
  ...
  applyRoleFilter(); cy.fit(cy.elements(), 40);
...
async function select(gid) {
  gid = String(gid);
  state.selected = gid;
  if (location.hash !== "#c=" + gid) history.pushState(null, "", "#c=" + gid);   // ссылка на клиента и кнопка «Назад»
  $("#search-results").classList.add("hidden");
  if (!$("#card .card")) $("#card").innerHTML = '<div class="empty"><p>Загрузка карточки…</p></div>';
  else $("#card .card").classList.add("loading");
  ...
  const cur = document.querySelector(".tab.active .item.sel"); if (cur) cur.scrollIntoView({ block: "nearest" });
  if (state.mode === "ego") await drawEgo(gid); else highlight(gid);
  renderCard(gid);
  if ($("#tab-chat").classList.contains("active")) renderChips();
}

// ---------- карточка узла ----------
async function renderCard(gid) {
  let n;
  try { n = await api("node/" + gid + "?ctx=card"); }
  catch (e) { toast("Клиент " + short(gid) + " не найден: " + e.message, "bad"); $("#card").innerHTML = emptyCard(); bindEmpty(); return; }
  if (state.selected !== gid) return;     // пользователь уже выбрал другого клиента
  const caveats = [];
  ...
  // дневная активность: поступления / списания
  ...
  const bars = days.map((d, k) => `<div style="height:${100 * d.i / mx}%;background:var(--in)" title="${k + 1} июля: поступления ${money(d.i)}"></div><div style="height:${100 * d.o / mx}%;background:var(--out)" title="${k + 1} июля: списания ${money(d.o)}"></div>`).join("");
  const rank = TOP.find(t => t.gid === gid);
  const stabCol = n.role_stability >= .9 ? "var(--in)" : n.role_stability >= .7 ? "var(--warn)" : "var(--out)";
  const M = (label, value, tip) => `<div${tip ? ` title="${tip}"` : ""}><small${tip ? ' class="term"' : ""}>${label}</small><b>${value}</b></div>`;

  $("#card").innerHTML = `<div class="card">
    <div class="card-sticky"><div class="card-head">
      <span class="num card-gid" title="Полный идентификатор клиента">${gid}</span>
      <button class="btn ghost sm card-close" id="card-close" aria-label="Закрыть">Закрыть</button>
    </div>
    <div class="card-tools">
      <button class="icon-btn" id="copy-gid" title="Скопировать gid в буфер обмена">копировать gid</button>
      <button class="icon-btn" id="copy-link" title="Скопировать ссылку: коллега откроет эту же карточку">ссылка</button>
      ${rank ? `<span class="rank-chip">№${rank.rank} из ${TOP.length}</span>` : ""}
      <button class="icon-btn next" id="card-next" title="Следующий непроверенный клиент из списка приоритетов (клавиша N)">следующий →</button>
    </div>
    <div class="card-actions">
      <button class="btn accent" id="btn-dossier" title="Инфографика по клиенту (открывается в новой вкладке)">Досье</button>
      <button class="btn" id="btn-ego" title="Показать плательщиков и получателей на графе">Окрестность</button>
      <button class="btn ghost" id="btn-pdf" title="Текстовая справка для печати или PDF">Справка</button>
      <button class="btn ghost" id="btn-ai" title="Краткое резюме от AI (нужен ключ OpenAI/NVIDIA)">AI-справка</button>
    </div></div>
    <div class="card-hero">
      <div><div class="big num">${n.priority_score.toFixed(2)}</div><small class="term" title="0–1: 35% роль, 25% объём денег, 20% охват seed, 20% центральность">приоритет</small></div>
      <div><div class="big num" style="color:${stabCol}">${Math.round(n.role_stability * 100)}%</div>
        <small class="term" title="В скольких из 60 вариантов порогов (каждый сдвинут на ±20%) роль остаётся той же">устойчивость роли</small></div>
      <div><div class="big num">${n.role_score.toFixed(2)}</div><small class="term" title="0.5 — ровно на пороге правила, 1.0 — максимум по сети">уверенность</small></div>
    </div>
    <div class="tags">${badge(n.role)} ${n.is_seed ? '<span class="badge seed" title="Клиент из исходного списка правоохранительных органов">seed</span>' : ""}
      <span title="Сколько переводов отделяет клиента от исходного списка">${pill("колено " + n.depth, "#9AA3AF")}</span> ${pill("кластер " + n.cluster_id, "#9AA3AF")}
    ...
    ${n.alt_role ? `<div class="stab">Роль пограничная в части вариантов порогов — иначе <b>${ROLE_RU_ALT(n.alt_role)}</b>.</div>` : ""}
    ...
    <div class="metrics">
      ${M("Поступило: плательщиков / сумма", `${n.in_deg} / ${money(n.in_sum)}`)}
      ${M("Отправлено: получателей / сумма", `${n.out_deg} / ${money(n.out_sum)}`)}
      ${M("Коэф. пропуска", pr, "Какая доля полученного ушла дальше (отправлено ÷ получено)")}
      ${M("Ушло дальше за ≤2 дня", Math.round(n.fast_share * 100) + "%", "Сквозной транзит: доля поступлений, переведённая дальше в течение 2 дней")}
      ${M("Seed выше по цепочке", n.seed_reach, "От скольких клиентов исходного списка деньги доходят до этого клиента")}
      ${M("Активных дней", n.active_days)}
    </div>
    <details class="more"><summary>Все показатели</summary><div class="metrics">
      ${M("Макс. плательщиков в день", n.max_payers_same_day, "Синхронные поступления от разных плательщиков")}
      ${M("Платят узлы-хабы", n.hub_payers, "Сколько сборщиков и распределителей платят клиенту")}
      ${M("Возвратные цепочки / встречные", `${n.cycles} / ${n.reciprocal}`, "Деньги возвращаются к отправителю за ≤4 шага / пары A⇄B")}
      ${M("Эпизоды дробления (отпр. / получ.)", `${n.split_out} / ${n.split_in}`, "≥3 перевода одному получателю в один день")}
      ${M("Доля переводов 5–10 тыс ₸", Math.round(n.near_threshold_share * 100) + "%", "Переводы у нижнего порога выгрузки")}
      ${M("Пиковый день · доля оборота", n.burst_day ? n.burst_day.slice(8) + ".07 · " + Math.round(n.burst_share * 100) + "%" : "—", "Какая доля месячного оборота пришлась на один день")}
    </div></details>
    <h3>Активность по дням (июль) — поступления / списания</h3><div class="bars">${bars}</div>
    <div id="cycles"></div>
    <div id="splits"></div>
    <div id="routes"></div>
    <h3>Входящие (${n.incoming.length})</h3><div class="flows">${flows(n.incoming, "←") || "<small class='note'>нет в выгрузке</small>"}</div>
    <h3>Исходящие (${n.outgoing.length})</h3><div class="flows">${flows(n.outgoing, "→") || "<small class='note'>нет в выгрузке</small>"}</div>
  </div>`;
  $("#copy-gid").onclick = () => copy(gid, "gid скопирован");
  $("#copy-link").onclick = () => copy(location.origin + "/#c=" + gid, "ссылка на клиента скопирована");
  ...
  $("#card-next").onclick = nextUnchecked;
  $("#card").scrollTop = 0;
  renderDecomp(n);
  ...
  const save = async status => {
    try {
      const r = await api("marks/" + gid, { method: "POST", headers: { "content-type": "application/json" },
        body: JSON.stringify({ status, comment: $("#mark-comment").value }) });
      await loadMarks();                 // перечитываем с ролью и приоритетом для вкладок статусов
      renderMarkBox(gid, r.mark); renderTop();
      toast(status === "clear" ? "Отметка снята" : `Сохранено: ${MARK[status].ru}`, "ok", status !== "clear" && { label: "Следующий →", fn: nextUnchecked });
    } catch (e) { toast("Не удалось сохранить: " + e.message, "bad"); }
  };
  $("#markbox").querySelectorAll("button").forEach(b => b.onclick = () => save(b.dataset.s));
  $("#mark-comment").onkeydown = e => {    // Enter — сохранить комментарий к текущему решению (по умолчанию «на проверке»)
    if (e.key === "Enter") { e.preventDefault(); save(st || "review"); }
  };
}
...
$("#chat-form").onsubmit = async e => {
  ...
  try {
    const r = await api("assistant", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ messages: chatHistory }) });
    ...
    bindLinks(pending);
  } catch (err) { pending.textContent = "Ошибка: " + err.message; chatHistory.pop(); }
  log.scrollTop = log.scrollHeight;
};
...
$("#search").onkeydown = e => {
  ...
  const q = e.target.value.replace(/\D/g, "");
  if (q.length >= 15) { $("#search-results").classList.add("hidden"); e.target.blur(); return select(q); }   // вставили полный gid
  const f = $("#search-results [data-gid]"); if (f) f.click();
};
...
function renderLegend() {
  $("#legend").innerHTML = state.color === "role"
    ? '<small class="lg-h">нажмите роль, чтобы скрыть</small>'
      + Object.entries(ROLES).map(([k, v]) => `<button data-role="${k}" class="${hiddenRoles.has(k) ? "off" : ""}"><i style="background:${v.color}"></i>${v.ru}</button>`).join("")
      + '<span><i style="box-shadow:inset 0 0 0 2px #EAECEF"></i>seed</span>'
      + (hiddenRoles.size ? '<button data-role="*" class="lg-reset">показать все</button>' : "")
    : PALETTE.map((c, i) => `<span><i style="background:${c}"></i>кластер ${i + 1}</span>`).join("") + '<span><i style="background:#4A5260"></i>прочие</span>';
  $("#legend").querySelectorAll("button[data-role]").forEach(b => b.onclick = () => {
    const r = b.dataset.role;
    if (r === "*") hiddenRoles.clear(); else hiddenRoles.has(r) ? hiddenRoles.delete(r) : hiddenRoles.add(r);
    applyRoleFilter(); renderLegend();
    if (state.mode === "full" && !state.selected) cy.animate({ fit: { eles: cy.nodes().not(".off"), padding: 30 } }, { duration: 300 });
  });
}
// фильтр ролей на схеме: скрытые роли не рисуются (выбранный клиент виден всегда)
const hiddenRoles = new Set();
function applyRoleFilter() {
  if (!cy) return;
  cy.batch(() => {
    cy.nodes().forEach(n => n.toggleClass("off", hiddenRoles.has(n.data("role")) && n.id() !== state.selected));
  });
}
...
// ---------- удобство: уведомления, копирование, ссылки, справка, клавиатура ----------
...
function toast(msg, kind = "", action = null) {
  ...
  if (action) {
    ...
    b.onclick = () => { t.remove(); action.fn(); }; t.append(b);
  }
  box.append(t);
  setTimeout(() => t.classList.add("out"), action ? 4200 : 2400);
  setTimeout(() => t.remove(), action ? 4600 : 2800);
}
async function copy(text, msg) {
  try { await navigator.clipboard.writeText(text); }
  catch (e) {             // http без разрешения — запасной путь через скрытое поле
    const ta = document.createElement("textarea"); ta.value = text; ta.style.position = "fixed"; ta.style.opacity = "0";
    document.body.append(ta); ta.select(); try { document.execCommand("copy"); } catch (e2) {} ta.remove();
  }
  toast(msg, "ok");
}
addEventListener("unhandledrejection", e => toast("Ошибка: " + (e.reason && e.reason.message || e.reason), "bad"));

// ссылка на клиента: /#c=<gid>; «Назад» в браузере возвращает к предыдущему клиенту
const gidFromHash = () => { const m = location.hash.match(/^#c=(\d{6,24})$/); return m ? m[1] : null; };
addEventListener("popstate", () => {
  const g = gidFromHash();
  if (g && g !== state.selected) select(g);
  else if (!g) { state.selected = null; $("#card").innerHTML = emptyCard(); bindEmpty(); closeCard(); if (cy) clearHL(); }
});

// следующий непроверенный клиент по списку приоритетов
function nextUnchecked() {
  const i = TOP.findIndex(t => t.gid === state.selected);
  const nxt = TOP.slice(i + 1).find(t => !MARKS.has(t.gid)) || TOP.find(t => !MARKS.has(t.gid));
  if (nxt) select(nxt.gid); else toast("Все клиенты из топ-листа проверены", "ok");
}

// пустая карточка: с чего начать
function emptyCard() {
  return `<div class="start">
    <img src="logo.svg" width="36" height="36" alt="">
    <h2>С чего начать</h2>
    <ol>
      <li><b>Откройте клиента</b> — первым в списке «Приоритеты» стоит тот, кого стоит проверить раньше всех. Можно кликнуть узел на схеме или найти gid (клавиша <kbd>/</kbd>).</li>
      <li><b>Прочитайте обоснование</b> — роль, почему она назначена и насколько вывод устойчив к порогам.</li>
      <li><b>Отметьте решение</b> — «на проверке», «подтверждено» или «отклонено» с комментарием. Статусы видны во вкладках списка и в журнале.</li>
    </ol>
    <div class="start-actions"><button class="btn accent" id="start-first">Открыть №1 из списка</button>
      <button class="btn ghost" id="start-help">Как пользоваться</button></div>
    <p class="note">Клавиши: <kbd>↑</kbd><kbd>↓</kbd> — по списку, <kbd>N</kbd> — следующий непроверенный, <kbd>Esc</kbd> — закрыть, <kbd>?</kbd> — справка.</p>
  </div>`;
}
function bindEmpty() {
  const f = $("#start-first"), h = $("#start-help");
  if (f) f.onclick = async () => { if (!TOP.length) await renderTop(); if (TOP[0]) select(TOP[0].gid); };
  if (h) h.onclick = openHelp;
}

// справка
function openHelp() { $("#help").classList.remove("hidden"); $("#help .help-body").scrollTop = 0; }
function closeHelp() { $("#help").classList.add("hidden"); }
$("#btn-help").onclick = openHelp;
$("#help-close").onclick = closeHelp;
$("#help").onclick = e => { if (e.target.id === "help") closeHelp(); };
$("#help-lab").onclick = () => { closeHelp(); if (isPhone()) setView("graph"); openLab(); };

// подсказки-вопросы для ассистента
const CHIPS = [
  "Кто пять самых приоритетных клиентов и почему?",
  "Опиши кластер 1: кто в нём и какая гипотеза?",
  "Что будет с сетью, если заблокировать топ-10?",
  "У кого есть признаки дробления платежей?",
  "Какие данные запросить в первую очередь?",
];
function renderChips() {
  const box = $("#chat-chips");
  const sel = state.selected ? [`Объясни роль клиента ${state.selected}`] : [];
  box.innerHTML = [...sel, ...CHIPS].map(q => `<button class="qchip">${esc(q.length > 60 ? q.replace(state.selected, short(state.selected)) : q)}</button>`).join("");
  box.querySelectorAll("button").forEach((b, i) => b.onclick = () => {
    $("#chat-input").value = [...sel, ...CHIPS][i]; $("#chat-form").requestSubmit();
  });
}
document.querySelector('[data-tab="chat"]').addEventListener("click", renderChips);
...
document.addEventListener("keydown", e => {
  ...
  if (e.key === "Escape") {
    ...
    if (!$("#search-results").classList.contains("hidden")) { $("#search-results").classList.add("hidden"); return $("#search").blur(); }
    if (typing) return document.activeElement.blur();
    if (!$("#lab").classList.contains("hidden")) return closeLab();
    if (!$("#player").classList.contains("hidden")) return $("#pl-close").click();
    if ($("#main").classList.contains("card-open")) return closeCard();
    return;
  }
  ...
  if (e.key === "/") { e.preventDefault(); $("#search").focus(); }
    ...
    const items = [...document.querySelectorAll(".tab.active .item[data-gid]")];
    if (!items.length) return;
    e.preventDefault();
    const i = items.findIndex(el => el.dataset.gid === state.selected);
    const d = e.key === "ArrowDown" || e.key === "j" ? 1 : -1;
    const next = items[i < 0 ? 0 : Math.min(items.length - 1, Math.max(0, i + d))];
    if (next.dataset.gid !== state.selected) select(next.dataset.gid);
  }
  else if (e.key === "n" || e.key === "N" || e.key === "т" || e.key === "Т") nextUnchecked();
  else if ((e.key === "e" || e.key === "у") && state.selected) $("#btn-ego") && $("#btn-ego").click();
  else if ((e.key === "d" || e.key === "в") && state.selected) $("#btn-dossier") && $("#btn-dossier").click();
});

(async () => {
  try { CFG = await api("config"); } catch (e) { /* без конфига просто не рисуем разложение */ }
  try { await loadMarks(); } catch (e) {}
  $("#card").innerHTML = emptyCard(); bindEmpty();
  renderStats(); renderClusters(); renderGaps(); renderLegend(); renderIntegrity(); renderChips();
  await Promise.all([renderTop(), initGraph()]);
  const g = gidFromHash();
  if (g) select(g);
  else if (!store("tamyr.helpSeen")) { openHelp(); store("tamyr.helpSeen", "1"); }
})();
